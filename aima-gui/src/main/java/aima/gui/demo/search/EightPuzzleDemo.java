...
//			new EightPuzzleBoard(new int[] { 0, 8, 7, 6, 5, 4, 3, 2, 1 });
	...
	public static void main(String[] args) {
		...
		//eightPuzzleAStarNullHeuristicDemo();						//h = 0
		//eightPuzzleAStarWeightedMisplacedDemo();					//h1
		//eightPuzzleAStarWeightedManhattamDemo();					//h2
	...
	private static void eightPuzzleAStarNullHeuristicDemo() {
		System.out.println("\nEightPuzzleDemo AStar Search (NullHeuristic)");
		try {
			Problem<EightPuzzleBoard, Action> problem = new BidirectionalEightPuzzleProblem(random1);
			SearchForActions<EightPuzzleBoard, Action> search = new AStarSearch<>
					(new GraphSearch<>(), EightPuzzleFunctions::getNullHeuristic);
			SearchAgent<Object, EightPuzzleBoard, Action> agent = new SearchAgent<>(problem, search);
			printActions(agent.getActions());
			printInstrumentation(agent.getInstrumentation());
		} catch (Exception e) {
	...
	private static void eightPuzzleAStarWeightedManhattamDemo() {
		System.out.println("\nEightPuzzleDemo AStar Search (NullHeuristic)");
		try {
			Problem<EightPuzzleBoard, Action> problem = new BidirectionalEightPuzzleProblem(random1);
			SearchForActions<EightPuzzleBoard, Action> search = new AStarSearch<>
	...
	private static void eightPuzzleAStarWeightedNonConsistentHeuristicDemo() {
		...
		try {
			Problem<EightPuzzleBoard, Action> problem = new BidirectionalEightPuzzleProblem(random1);
			SearchForActions<EightPuzzleBoard, Action> search = new AStarSearch<>
					(new GraphSearch<>(), EightPuzzleFunctions::getNonWeigthedConsistentHeuristic);
			SearchAgent<Object, EightPuzzleBoard, Action> agent = new SearchAgent<>(problem, search);
			printActions(agent.getActions());
			printInstrumentation(agent.getInstrumentation());
		} catch (Exception e) {
	...
	private static void eightPuzzleAStarEpsilonWeigthedManhattanDemo() {
		...
		try {
			Problem<EightPuzzleBoard, Action> problem = new BidirectionalEightPuzzleProblem(random1);
			SearchForActions<EightPuzzleBoard, Action> search = new AStarSearch<>
					(new GraphSearch<>(), EightPuzzleFunctions::getEpsiloneWeigthManhattamDistance);
			SearchAgent<Object, EightPuzzleBoard, Action> agent = new SearchAgent<>(problem, search);
			printActions(agent.getActions());
			printInstrumentation(agent.getInstrumentation());
		} catch (Exception e) {
